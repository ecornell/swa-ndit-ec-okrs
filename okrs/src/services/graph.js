...
export default {
    ...
    async getList(list, select = '', filter = '', top = 999, fieldsOnly = true) {
        ...
        let call = `/sites/${siteId}/lists/${list}/items?`

        if (select === '') {
            call = call + `expand=fields`
        } else {
            call = call + `expand=fields(select=${select})`
        }
...
async function callGraph(apiPath) {
    ...
    if (!resp.ok) {
        ...
        console.error(error)
        throw new Error(`Call to ${GRAPH_BASE}${apiPath} failed: ${resp.statusText} error: ${error.error.message}`)
    }
